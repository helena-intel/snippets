...
ov_model_id = "bge-small-en-v1.5"
ov_model_id_static = "bge-small-en-v1.5-static"
ov_model_id_static_norm = "bge-small-en-v1.5-static-norm"
...
ov_model = ov.Core().compile_model(f"{ov_model_id_static}/openvino_model.xml", device_name=device, config=ov_config)
...
ov_tokenizer = ov.Core().compile_model(f"{ov_model_id_static}/openvino_tokenizer.xml", device_name="CPU")
...
ov_model = ov.Core().compile_model(f"{ov_model_id_static_norm}/openvino_model.xml", device_name=device, config=ov_config)
ov_tokenizer = ov.Core().compile_model(f"{ov_model_id_static_norm}/openvino_tokenizer.xml", device_name="CPU")
ov_encoded_input = ov_tokenizer(sentences)
...
print("PyTorch                                    :", np.array(pt_sentence_embeddings[0, :5]).round(4))
print("OpenVINO Optimum                           :", np.array(opt_sentence_embeddings[0, :5]).round(4))
print("OpenVINO Base + HF Tokenizers              :", ov_sentence_embeddings[0, :5].round(4))
print("OpenVINO Base + OV Tokenizers              :", ov_sentence_embeddings2[0, :5].round(4))
print("OpenVINO Base with norm + OV Tokenizers    :", ov_sentence_embeddings3[0, :5].round(4))
